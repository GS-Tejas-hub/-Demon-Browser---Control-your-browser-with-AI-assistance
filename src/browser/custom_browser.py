...
class CustomBrowser(Browser):
    ...
    async def _setup_custom_browser(self, playwright: Playwright) -> PlaywrightBrowser:
        """Sets up and returns a Playwright Browser instance using a custom browser binary."""
        assert self.config.browser_binary_path is not None, 'browser_binary_path should not be None for custom browser setup'
        
        logger.info(f"Launching custom browser with binary: {self.config.browser_binary_path}")
        logger.info(f"Extra browser args: {self.config.extra_browser_args}")
